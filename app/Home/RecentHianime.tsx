...
export default function AnimeEpisodesSlider() {
  ...
  useEffect(() => {
    axios.get('https://tae-pied.vercel.app/api/')
      .then(async (response) => {
        // Data is at response.data.results.latestEpisode
        const raw: any[] = response.data?.results?.latestEpisode ?? [];

        if (raw.length === 0) {
          console.warn('No latestEpisode array found:', response.data);
          setLoading(false);
          return;
        }

        const results: Episode[] = raw.map((item: any) => ({
          id: item.id ?? '',
          ...
          sub: parseInt(item.tvInfo?.sub ?? '0', 10),
          dub: parseInt(item.tvInfo?.dub ?? '0', 10),
        }));

        // Show episodes immediately
        setEpisodes(results);
        setLoading(false);

        // Enrich with details in the background
        results.forEach(async (episode, index) => {
          if (!episode.id) return;
          try {
            const detailsResponse = await axios.get(
              `https://noone-bice.vercel.app/api/hianime/anime/${episode.id}`
            );
              ...
              updated[index] = {
                ...
                animeDetails: {
                  latestEpisode: latestEpisode
                    ? { episodeId: latestEpisode.episodeId, episodeNumber: latestEpisode.episodeNumber }
                    : undefined,
            ...
            console.error(`Details fetch failed for ${episode.id}:`, err);
          }
        });
      })
      .catch(err => {
        ...
        <div className="relative px-8 md:px-12">
          ...
          <div className="overflow-hidden rounded-xl">
            <div
              ...
              style={{ transform: `translateX(-${boundedIndex * (100 / visibleItems)}%)` }}>
  ...
  if (episode.animeDetails?.latestEpisode?.episodeId && episode.animeDetails?.anilistId) {
    ...
    window.location.href = `/Watch/hianime/${episode.id}?animeId=${episode.animeDetails?.anilistId ?? ''}`;
  }
}}>
                    ...
                    <div className="relative aspect-square overflow-hidden">
                      ...
                      <div className="absolute bottom-3 right-3 bg-gradient-to-r from-red-600 to-red-700 text-white px-3 py-1.5 rounded-full text-xs md:text-sm font-bold shadow-lg">
                        {episode.animeDetails?.latestEpisode?.episodeNumber
                          ...
                          : episode.sub > 0 ? `SUB ${episode.sub}` : episode.type || 'TV'}
                      </div>
                    ...
                    <div className="p-3 md:p-4">
                      ...
                      <div className="flex items-center justify-between mt-2 text-xs">
                        {episode.japaneseTitle && (
                          <p className="text-gray-400 line-clamp-1 flex-1">{episode.japaneseTitle}</p>
                        )}
