...
function AnimeKaiPlayer() {
  ...
  useEffect(() => {
    ...
    const handleProgress = () => {
      if (video.buffered.length > 0) {
        ...
        if (video.duration > 0) setBuffered((end / video.duration) * 100);
      }
    };
    ...
    const loadVideo = () => {
      if (window.Hls && window.Hls.isSupported()) {
        ...
        const hls = new window.Hls({
          ...
          highBufferWatchdogPeriod: 2, nudgeOffset: 0.1, nudgeMaxRetry: 3,
          maxFragLookUpTolerance: 0.25, liveSyncDurationCount: 3,
          liveMaxLatencyDurationCount: Infinity, liveDurationInfinity: false,
          enableWebVTT: true, enableIMSC1: true, enableCEA708Captions: true,
          stretchShortVideoTrack: false, maxAudioFramesDrift: 1,
          forceKeyFrameOnDiscontinuity: true, abrEwmaFastLive: 3.0,
          abrEwmaSlowLive: 9.0, abrEwmaFastVoD: 3.0, abrEwmaSlowVoD: 9.0,
          abrEwmaDefaultEstimate: 500000, abrBandWidthFactor: 0.95,
          abrBandWidthUpFactor: 0.7, abrMaxWithRealBitrate: false,
          maxStarvationDelay: 4, maxLoadingDelay: 4, minAutoBitrate: 0,
          enableSoftwareAES: true, manifestLoadingTimeOut: 10000,
          manifestLoadingMaxRetry: 1, manifestLoadingRetryDelay: 1000,
          manifestLoadingMaxRetryTimeout: 64000, startLevel: undefined,
          levelLoadingTimeOut: 10000, levelLoadingMaxRetry: 4,
          levelLoadingRetryDelay: 1000, levelLoadingMaxRetryTimeout: 64000,
          fragLoadingTimeOut: 20000, fragLoadingMaxRetry: 6,
          fragLoadingRetryDelay: 1000, fragLoadingMaxRetryTimeout: 64000,
  ...
  useEffect(() => {
    ...
    styleElement.textContent = `
      ...
      @media (max-width: 640px) {
        video::cue { font-size: calc(0.7rem + 0.8vw); }
        video::-webkit-media-text-track-container { bottom: 20% !important; padding: 0 3% !important; }
      }
      @media (min-width: 641px) and (max-width: 1024px) {
        video::cue { font-size: calc(0.75rem + 0.6vw); }
        video::-webkit-media-text-track-container { bottom: 16% !important; }
      }
    `;
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (videoRef.current) { videoRef.current.volume = vol; setVolume(vol); setIsMuted(vol === 0); }
  };
              ...
              {videoLoading && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-50">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-gray-300 text-sm">Please wait while the stream initializes</p>
                  </div>
                </div>
              )}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 space-y-2 sm:space-y-4 pointer-events-auto">
                  ...
                  <div className="relative w-full h-1 sm:h-2 bg-gray-600 rounded-full overflow-hidden cursor-pointer group">
                    <div className="absolute inset-y-0 left-0 bg-gray-500" style={{ width: `${buffered}%` }} />
                    <div className="absolute inset-y-0 left-0 bg-red-600 group-hover:bg-red-500 transition-colors" style={{ width: `${(currentTime / duration) * 100}%` }} />
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 sm:gap-4">
                      <button onClick={togglePlay} className="w-8 h-8 sm:w-10 sm:h-10 hover:bg-gray-700/50 rounded-lg flex items-center justify-center transition-colors touch-manipulation">
                        {isPlaying ? <PauseIcon /> : <PlayIcon />}
                      </button>

                      <div className="flex items-center gap-2">
                        <button onClick={toggleMute} className="w-8 h-8 sm:w-10 sm:h-10 hover:bg-gray-700/50 rounded-lg flex items-center justify-center transition-colors touch-manipulation">
                        ...
                        <input
                          ...
                          style={{ background: `linear-gradient(to right, #ef4444 0%, #ef4444 ${volume * 100}%, #4b5563 ${volume * 100}%, #4b5563 100%)` }}
                        />
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {sourcesData?.data.download && (
                        <a href={sourcesData.data.download} download className="px-2 py-1 sm:px-3 sm:py-1.5 bg-gray-800/80 hover:bg-gray-700 rounded-lg text-xs font-semibold transition-all touch-manipulation flex items-center gap-1" title="Download">
                          <DownloadIcon />
                        </a>
                      )}
                      ...
                      <button onClick={toggleFullscreen} className="w-8 h-8 sm:w-10 sm:h-10 hover:bg-gray-700/50 rounded-lg flex items-center justify-center transition-colors touch-manipulation" title="Fullscreen">
          ...
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-4 sm:p-6 shadow-2xl border border-gray-700 mb-6">
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
              ...
              {(() => {
                ...
                return prev ? (
                  <button onClick={() => handleEpisodeChange(prev.episodeId)} className="flex items-center justify-center bg-black hover:bg-red-900/30 p-3 sm:p-4 rounded-xl transition-all transform hover:scale-105 border border-red-900/50 min-w-[100px]">
                    <BackIcon />
                  ...
                  <div className="flex items-center justify-center bg-gray-800/50 p-3 sm:p-4 rounded-xl min-w-[100px] text-gray-500 text-sm">No Previous</div>
                );
              })()}
    ...
    .join('-')}
              ...
              {(() => {
                ...
                return next ? (
                  ...
                  <div className="flex items-center justify-center bg-gray-800/50 p-3 sm:p-4 rounded-xl min-w-[100px] text-gray-500 text-sm">No Next</div>
                );
              })()}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-4 sm:p-6 shadow-2xl border border-gray-700">
            ...
            {loadingEpisodes ? (
              ...
              <div className="text-center py-12 space-y-4">
                ...
                <button onClick={() => { setSearchQuery(""); setEpisodeRange("all"); }} className="mt-4 px-4 sm:px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm sm:text-base">
                  Clear Filters
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                {filteredEpisodes.map((episode) => (
                  <button
                    ...
                    className={`group text-left p-2 sm:p-3 rounded-lg sm:rounded-xl transition-all transform hover:scale-105 hover:shadow-xl ${
