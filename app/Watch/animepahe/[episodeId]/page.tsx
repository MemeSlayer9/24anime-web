...
function AnimePahePlayer() {
  ...
  const [showEpisodesList] = useState(true);
  ...
  useEffect(() => {
    async function fetchEpisodes() {
      ...
      try {
        ...
        if (foundTitle) setAnimeTitle(foundTitle);
      } catch (err) {
...
useEffect(() => {
  ...
  useEffect(() => {
    if (!proxiedVideoUrl || !videoRef.current || playerMode === 'iframe') return;
...
const handleServerChange = (server: Server) => {
  ...
  if (error && !error.includes('Dub version is not available')) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white flex items-center justify-center p-3 sm:p-4">
        <div className="text-center space-y-4 sm:space-y-6 max-w-md px-4">
