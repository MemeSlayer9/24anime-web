...
function AnimePahePlayer() {
  ...
  const parseM3U8Manifest = async (url: string): Promise<QualityLevel[]> => {
    try {
    ...
    async function fetchEpisodes() {
      ...
      try {
        ...
        const response = await fetch(
          `https://kenjitsu.vercel.app/api/anilist/episodes/${animeId}?provider=anizone`
        );
  ...
  useEffect(() => {
    async function fetchSources() {
      ...
      try {
        ...
        const response = await fetch(
          `https://kenjitsu.vercel.app/api/anizone/sources/${encodeURIComponent(episodeId)}?version=${version}&server=hd-2`
        );
        ...
        if (data.data.sources && data.data.sources.length > 0) {
          const m3u8Source = data.data.sources.find((s: VideoSource) => s.isM3u8);
          if (m3u8Source) {
            const proxyUrl = `https://proxys.ciphertv.dev/proxy?url=${encodeURIComponent(m3u8Source.url)}`;
            const levels = await parseM3U8Manifest(proxyUrl);
  ...
  const selectedLevel = qualityLevels.find(level => level.quality === selectedQuality);
  const videoUrl = selectedLevel?.url || '';
  ...
  useEffect(() => {
    ...
    // Determine which subtitle to show by default
    const defaultSubtitle = subtitles.find(s => s.default)?.lang || subtitles[0]?.lang;
    
    // Set selected subtitle if not set
    ...
    subtitles.forEach((subtitle) => {
      ...
      // Set default attribute for the selected subtitle
    ...
    // Force subtitle activation with enhanced mobile support
    const activateSubtitle = () => {
      ...
      // If we didn't activate any track, force the first one
      if (!activated && tracks.length > 0) {
        tracks[0].mode = 'showing';
      }

      // Additional mobile-specific activation
    ...
    // Multiple delayed activations with increasing delays - extended for mobile
    ...
    const trackLoadHandler = () => {
      activateSubtitle();
      // Additional activation after a short delay on mobile
      if ('ontouchstart' in window) {
    ...
    const loadVideo = () => {
      if (window.Hls && window.Hls.isSupported()) {
        ...
        hls.on(window.Hls.Events.MANIFEST_PARSED, () => {
          if (selectedQuality !== 'Auto' && hls.levels && hls.levels.length > 0) {
            const qualityIndex = qualityLevels.findIndex(l => l.quality === selectedQuality);
            if (qualityIndex > 0 && qualityIndex - 1 < hls.levels.length) {
  ...
  }, [videoUrl, selectedQuality, qualityLevels]);
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="px-2 py-1 sm:px-3 sm:py-1.5 bg-gray-800/80 hover:bg-gray-700 rounded-lg text-xs font-semibold transition-all touch-manipulation"
                        disabled={loadingQualities}
                      >
                        {loadingQualities ? 'Loading...' : selectedQuality}
                      </button>
                      {showQualityMenu && qualityLevels.length > 0 && (
                        <div className="absolute bottom-full right-0 mb-2 bg-gray-800 rounded-lg shadow-2xl overflow-hidden min-w-[120px] max-h-[300px] overflow-y-auto z-50">
                          {qualityLevels.map((level) => (
                            <button
                              key={level.quality}
                              onClick={() => handleQualityChange(level.quality)}
                              className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-700 transition-colors ${
                                selectedQuality === level.quality ? 'bg-red-600 text-white' : 'text-gray-300'
                              }`}
                            >
                              {level.quality}
                              {level.resolution && level.quality !== 'Auto' && (
                                <span className="text-xs text-gray-400 ml-2">({level.resolution})</span>
                              )}
                            </button>
                          ))}
                        </div>
                ...
                {filteredEpisodes.map((episode) => (
                  ...
                  >
                    <div className="space-y-2">
                      {episode.thumbnail && (
                        <img
                          src={getWeservImage(episode.thumbnail, 300)}
                          alt={episode.title}
