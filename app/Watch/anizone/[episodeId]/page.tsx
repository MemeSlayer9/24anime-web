...
// Helper function to clean thumbnail URLs
const cleanThumbnailUrl = (url: string): string => {
  if (!url) return '';
  // Remove leading/trailing quotes and underscores
  return url.replace(/^['"]?_*/, '').replace(/_*['"]?$/, '').trim();
};
...
function AnimePahePlayer() {
  ...
  const controlsTimeoutRef = useRef<NodeJS.Timeout | null>(null);
   ...
   useEffect(() => {
    ...
    if (animeTitle && currentEp) {
      ...
      const cleanedThumbnail = cleanThumbnailUrl(currentEp.thumbnail);
      if (cleanedThumbnail) {
        const ogImage = document.querySelector('meta[property="og:image"]') || document.createElement('meta');
        ogImage.setAttribute('property', 'og:image');
        ogImage.setAttribute('content', cleanedThumbnail);
      ...
      if (cleanedThumbnail) {
        ...
        twitterImage.setAttribute('content', cleanedThumbnail);
  ...
  useEffect(() => {
    async function fetchEpisodes() {
      ...
      try {
        ...
        // Clean thumbnail URLs in the episodes data
        const cleanedEpisodes = (data.providerEpisodes || []).map(episode => ({
          ...episode,
          thumbnail: cleanThumbnailUrl(episode.thumbnail)
        }));
  ...
  useEffect(() => {
    async function fetchSources() {
      if (!episodeId || playerMode === 'embed') return;
  ...
  useEffect(() => {
    if (!videoRef.current || playerMode === 'embed') return;
  ...
  }, [subtitles, selectedSubtitle, videoUrl, playerMode]);

  useEffect(() => {
    if (!videoUrl || !videoRef.current || playerMode === 'embed') return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white">
      <div className="container mx-auto px-2 sm:px-4 py-3 sm:py-6 space-y-3 sm:space-y-6">
        {/* Player Mode Toggle - Above Video */}
        ...
        <div
          ...
          onMouseMove={playerMode === 'video' ? resetControlsTimeout : undefined}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-1 bg-gray-800/80 rounded-lg p-1">
                        <button
                          ...
                          } ${playerMode === 'video' ? 'opacity-50 cursor-not-allowed' : ''}`}
                          disabled={playerMode === 'video'}
                        >
                          SUB
                        </button>
                        <button
                      ...
                      <div className="relative">
                        <button
                          ...
                          className={`px-2 py-1 sm:px-3 sm:py-1.5 bg-gray-800/80 hover:bg-gray-700 rounded-lg text-xs font-semibold transition-all touch-manipulation flex items-center gap-1 ${
                            playerMode === 'embed' ? 'opacity-50 cursor-not-allowed' : ''
                          }`}
                          title="Subtitles"
                          disabled={playerMode === 'embed'}
                        >
                       <SubtitleIcon /> 
                        </button>
                        {showSubtitleMenu && playerMode === 'video' && (
          ...
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-4 sm:p-6 shadow-2xl border border-gray-700 mb-6">
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
              ...
              <div className="flex-1 flex flex-col sm:flex-row gap-3 sm:gap-6 bg-gradient-to-br from-red-950/50 to-black/50 p-3 sm:p-4 rounded-xl border border-red-900/50">
                {currentEpisode.thumbnail && cleanThumbnailUrl(currentEpisode.thumbnail) && (
                  <img
                    src={cleanThumbnailUrl(currentEpisode.thumbnail)}
            ...
            {loadingEpisodes ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                {filteredEpisodes.map((episode) => {
                  const cleanedThumbnail = cleanThumbnailUrl(episode.thumbnail);
                  return (
                    <button
